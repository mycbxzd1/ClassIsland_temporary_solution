...
/*
    在目录下输入compose pull拉取vendor/autoload.php
*/
...
if (!$api) {
    ...
    echo json_encode(['error' => 'no api'], JSON_PRETTY_PRINT);
    exit;
}
...
switch ($api) {
    case 'register':
        handleRequest(function() use ($pdo, $adminPassword) {
            if (empty($_GET['pwd']) || $_GET['pwd'] !== $adminPassword) {
                ...
                echo json_encode(['error' => 'Invalid registration password'], JSON_PRETTY_PRINT);
                return;
            }
            $name = $_GET['name'] ?? null;
            ...
            if (!$name || !$password) {
                http_response_code(400);
                echo json_encode(['error' => 'Name and password are required'], JSON_PRETTY_PRINT);
                return;
            }
            ...
            if ($user) {
                ...
                echo json_encode(['message' => 'User registered successfully'], JSON_PRETTY_PRINT);
            }
        });
        break;
    ...
    case 'refresh':
        handleRequest(function() use ($pdo) {
            ...
            echo json_encode(['message' => 'Tokens refreshed for all users'], JSON_PRETTY_PRINT);
        });
        break;
    ...
    case 'get':
        handleRequest(function() use ($pdo) {
            ...
            echo json_encode(['token' => $token, 'value' => $user['value']], JSON_PRETTY_PRINT);
        });
        break;
    ...
    case 'check':
        handleRequest(function() use ($pdo) {
            ...
            if (!$token) {
                http_response_code(400);
                echo json_encode(['error' => 'Token is required'], JSON_PRETTY_PRINT);
                return;
            }
            ...
            if ($tokenRecord) {
                ...
                if (time() - $timestamp > 180) { // 超过3分钟
                    ...
                    echo json_encode(['valid' => true, 'value' => $tokenRecord['value']], JSON_PRETTY_PRINT);
                } else {
                    echo json_encode(['valid' => true, 'value' => $tokenRecord['value']], JSON_PRETTY_PRINT);
                }
            } else {
                echo json_encode(['valid' => false], JSON_PRETTY_PRINT);
            }
        });
        break;
    ...
    case 'delete':
        handleRequest(function() use ($pdo, $adminPassword) {
            if (empty($_GET['pwd']) || $_GET['pwd'] !== $adminPassword) {
                http_response_code(403);
                echo json_encode(['error' => 'Invalid deletion password'], JSON_PRETTY_PRINT);
                return;
            }
            $name = $_GET['name'] ?? null;
            if (!$name) {
            ...
            $stmt->execute([$name]);
            echo json_encode(['message' => 'User and associated token deleted successfully'], JSON_PRETTY_PRINT);
        });
        break;
    ...
    case 'change':
        handleRequest(function() use ($pdo, $adminPassword) {
            ...
            if (!$name || (!$password && $value === null)) {
                http_response_code(400);
                echo json_encode(['error' => 'Name and either password or value are required'], JSON_PRETTY_PRINT);
                return;
            }
            if ($password) {
                $hashedPassword = hashPassword($password);
            ...
            echo json_encode(['message' => 'User information updated successfully'], JSON_PRETTY_PRINT);
        });
        break;

    default:
        http_response_code(400);
        echo json_encode(['error' => 'Invalid API action'], JSON_PRETTY_PRINT);
}
